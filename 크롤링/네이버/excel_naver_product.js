...
const makeFile = async (workbook) => {
  await workbook.xlsx.writeFile(`./네이버/상품리스트.xlsx`);
};

const makeNaverProductList = (productData) => {
...
const getNaverProductList = async () => {
  const Excel = require("exceljs");
  const workbook = new Excel.Workbook();

  const product_list = [];
  await workbook.xlsx.readFile("./네이버/상품리스트.xlsx").then((res) => {
...
module.exports = {
  makeNaverProductList: makeNaverProductList,
  getNaverProductList: getNaverProductList,
};
