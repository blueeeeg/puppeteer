...
const getInfo = async (page, product_count) => {
  ...
  return { name: product_name, url: product_url };
};
...
const NaverPriceComparisonProductCrawling = async () => {
  try {
    ...
    // 해당 url로 이동
    const url =
      "https://search.shopping.naver.com/search/all?frm=NVSHMDL&origQuery=%EB%8D%94%EB%9E%A9%EB%B0%94%EC%9D%B4%EB%B8%94%EB%9E%91%EB%91%90&pagingIndex=1&pagingSize=40&productSet=model&query=%EB%8D%94%EB%9E%A9%EB%B0%94%EC%9D%B4%EB%B8%94%EB%9E%91%EB%91%90&sort=price_asc&timestamp=&viewType=list";
    await page.goto(url);
    await page.evaluate(() => {
