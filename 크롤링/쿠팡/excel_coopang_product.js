...
const makeFile = async (workbook) => {
  await workbook.xlsx.writeFile(`./쿠팡/상품리스트.xlsx`);
};

const makeCoopangProductList = (productData) => {
...
const getCoopangProductList = async () => {
  const Excel = require("exceljs");
  const workbook = new Excel.Workbook();

  const product_list = [];
  await workbook.xlsx.readFile("./쿠팡/상품리스트.xlsx").then((res) => {
...
module.exports = {
  makeCoopangProductList: makeCoopangProductList,
  getCoopangProductList: getCoopangProductList,
};
